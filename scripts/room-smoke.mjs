...
try {
  ...
  assert.equal(await page.evaluate(() => JSON.parse(localStorage.getItem('bdc-room-v1')).placed[0].rotation), 15);
  await shop.getByRole('button', { name: 'Move', exact: true }).click();
  await page.mouse.click(canvas.x + canvas.width / 2, canvas.y + canvas.height / 2, { button: 'right' });
  await page.mouse.click(canvas.x + canvas.width / 2, canvas.y + canvas.height / 2);
  assert.equal(await page.evaluate(() => JSON.parse(localStorage.getItem('bdc-room-v1')).placed[0].rotation), 15);
  await shop.getByRole('button', { name: 'Colors', exact: true }).click();
  await shop.getByRole('button', { name: 'wall Sage', exact: true }).click();
  await shop.getByRole('button', { name: 'Tiles', exact: true }).last().click();
  await page.waitForFunction(() => JSON.parse(localStorage.getItem('bdc-room-v1')).appearance?.floorDesign === 'Tiles');
  await page.reload(); await page.locator('canvas').waitFor();
  assert.equal(await page.evaluate(() => JSON.parse(localStorage.getItem('bdc-room-v1')).appearance.wallColor), '#b9cbb0');
  await shop.screenshot({ path: 'runtime-debug/finishes.png' });
  await shop.getByRole('button', { name: 'Inventory', exact: true }).click();
  ...
  await page.mouse.click(canvas.x + canvas.width / 2, canvas.y + canvas.height / 2, { button: 'right' });
  await page.mouse.click(canvas.x + 220 / 440 * canvas.width, canvas.y + 340 / 500 * canvas.height);
  assert.equal(await page.evaluate(() => JSON.parse(localStorage.getItem('bdc-room-v1')).placed.length), 0, 'Right-click must cancel placement');
